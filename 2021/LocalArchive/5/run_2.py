...
for index in range(len(start)):
    s = start[index] if start[index][0] < end[index][0] else end[index]
    e = end[index] if start[index][0] < end[index][0] else start[index]
    if s[0] == e[0]:
        ...
        for val in range(min(s[0],e[0]), max(s[0], e[0]) + 1):
            grid[val][s[1]] += 1
    elif e[0] - s[0] == e[1] - s[1]:
        for val in range(e[0] - s[0] + 1):
            x = s[0] + val
            y = s[1] + val
            grid[x][y] += 1
    elif e[0] - s[0] == s[1] - e[1]:
        for val in range(e[0] - s[0] + 1):
            x = s[0] + val
            y = s[1] - val
            grid[x][y] += 1
